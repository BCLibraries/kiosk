var Stacks = ( function() {
		...
		function popUps() {
			...
			$("#" + listID).children("[id^=column]").children().children().each(function() {
				...
				$("[id^=Floor]").find("#stacks__" + text).on("click", function() {
					...
					//console.log(document.getElementById("#" + this.id).getBBox())
					//d3.select(this).append("rect").attr("height", document.getElementById("#" + this.id).getBBox()).attr("width",  document.getElementById("#" + this.id).getBBox())
		...
		function turnOffPopUp(toggleID) {
			...
			setTimeout(function() {d3.select("#popup_stacks").remove()}, .25 * 1000);
